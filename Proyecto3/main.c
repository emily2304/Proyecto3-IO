...
void* ReemplazoEquipos(){
	char * command = "gcc ReemplazoEquipos.c $(pkg-config --cflags --libs gtk+-3.0) -lm -export-dynamic -o ReemplazoEquipos";
	system(command);
...
void on_button3_clicked (GtkButton *button3, gpointer data){
	pthread_t thread;
	pthread_create(&thread, NULL, ReemplazoEquipos, NULL);
}
...
int main (int argc, char *argv[]){
	...
	gtk_widget_set_tooltip_text(button3, "The Equipment-Replacement problem's purpose is to find the best moment to keep or replace an asset that loses value and deteriorates over time (for example: a bycicle, a car, a machine, etc...). Usually, resale prices decrease and cost prices increase.\nFor example: Let's say you bought a motorcycle for delivery purposes, and you plan to work in Uber Eats for the next 5 years. The motorcycle's lifespan is 4 years, so you can't wait to sell it once the 5 years are over. As time passes, the resale prices decrease and maintenance cost increases. So, what would be the best option for you to make sure that you make the most of it and lose the less amount of money? Replace it in 2 years and buy a new one for the next 3 years of your delivery job? This is exactly the type of problem that the Equipment-Replacement algorithm tries to solve via Dynamic Programming.");
